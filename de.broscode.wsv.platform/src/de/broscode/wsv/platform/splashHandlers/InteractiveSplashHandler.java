...
public class InteractiveSplashHandler extends AbstractSplashHandler {
	...
	private void handleButtonOKWidgetSelected() {
		  ...
		  Thread t = new Thread() {
		    public void run() {
		      ...
		      if( ClientServiceLocator.instance().getUserService().doLogin(username, password) ) {
		    	SecurityContextImpl sc = new SecurityContextImpl();
			    Authentication auth = new UsernamePasswordAuthenticationToken(username, password);//DigestUtils.md5DigestAsHex(password.getBytes()));
			    sc.setAuthentication(auth);
			    SecurityContextHolder.setContext(sc);
		    	if(!ClientServiceLocator.instance().getMigrationService().migrate())
		    		loginStatus = 2;
	...
	private boolean login(String username, String password) {
		...
		// any password
		boolean retVal;
		retVal = ClientServiceLocator.instance().getUserService().doLogin(username, password);
		System.out.println("logged in: " + String.valueOf(retVal));
		if(retVal) {
			ClientServiceLocator.instance().getMigrationService().migrate();
		}
...
//		}
	...
	// ACHTUNG: Kann später entfernt werden! - Wird nur in der Methode login() verwendet
